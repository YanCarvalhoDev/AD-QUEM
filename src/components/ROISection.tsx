  ...
  const stats = [
    {
      ...
      title: "Visibilidade",
      description:
        "A forma como uma banca é encontrada nos mecanismos de busca e nas novas plataformas de descoberta influencia diretamente sua presença no ambiente digital.",
      detail:
        "83% dos decisores B2B pesquisam fornecedores de forma autônoma antes do contato. (Fonte: Gartner)",
    },
    {
      icon: <TrendingUp className="text-gold-pale" size={24} />,
      title: "Credibilidade",
      description:
        "Conteúdo especializado, autoria identificada e referências qualificadas ajudam a construir sinais de expertise e confiança no ambiente digital.",
      detail:
        "92% dos diretores jurídicos usam artigos como critério de contratação. (Fonte: BTI Consulting)",
    },
    {
      icon: <Zap className="text-gold-pale" size={24} />,
      title: "Experiência",
      description:
        "Performance, clareza e facilidade de acesso influenciam a percepção que potenciais clientes formam de uma organização durante sua jornada digital.",
      detail:
        "57% dos usuários não contratam empresas com sites lentos ou mal projetados. (Fonte: Deloitte)",
    },
  ];
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-20 gap-8">
          <div className="max-w-3xl">
            ...
            <motion.h2
              ...
              className="text-3xl sm:text-4xl lg:text-6xl font-bold text-teal-deep leading-tighter tracking-normal mb-8"
            >
              Autoridade também se mede: <br />
              <span className="text-gold-pale font-serif italic font-normal">
                dados por trás da percepção
              </span>
            </motion.h2>
          </div>
          {/* <motion.div
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-gray-100 border border-gray-100 rounded-2xl overflow-hidden shadow-2xl shadow-gray-200/50">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <h3 className="text-xl font-serif font-bold text-teal-deep mb-4 flex items-center gap-2 tracking-wider">
                {stat.title}
                <ArrowUpRight
                  size={16}
                  className="opacity-0 group-hover:opacity-100 transition-opacity text-gold-pale"
